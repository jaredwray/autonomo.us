/**
 * Refreshes `src/models.json`, the multi-provider model catalog served at
 * GET /v1/models/catalog. For each provider the script asks the provider's
 * own models endpoint when an API key is available in the environment, and
 * falls back to the public models.dev catalog (no key required) otherwise,
 * so a run with zero secrets still tracks every provider. The file is only
 * rewritten when the catalog content actually changed, keeping the daily
 * `update-models` workflow diff-free on quiet days.
 *
...
function countModels(catalog: ModelCatalog): number {
...
function changeSummary(changes: CatalogChange[]): string[] {
...
export async function main(): Promise<void> {
	...
	const stats = `${Object.keys(catalog.providers).length} providers, ${countModels(catalog)} models`;
	...
	for (const line of changeSummary(changes)) {
		console.log(`  ${line}`);
	}
